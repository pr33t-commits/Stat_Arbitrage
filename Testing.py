...
class FnOBacktester:
    ...
    def __init__(self, initial_capital: float, ohlcv_data: pd.DataFrame, margin_rules: dict, preprocessing_required = False):
        ...
        self.margin_maintained = 0 # e.g., {'NIFTY_FUT': 20000}
        # self.total_margin_maintained: float = 0.0
        self.current_position = {} # e.g., {'NIFTY_FUT': {'type': 'long', 'qty': 10, 'entry_price': 18000}, 'NIFTY_CE_18100': {'type': 'short', 'qty': 10, 'entry_price': 50}}
        self.proposed_position = {} # e.g., {'NIFTY_FUT': {'type': 'long', 'qty': 10, 'entry_price': 18000}, 'NIFTY_CE_18100': {'type': 'short', 'qty': 10, 'entry_price': 50}}
    ...
    def preprocess(self):
        ...
        self.expiry_mapper = retrieve_expiry_dates_df.groupby(col_dict['date_col']).apply(lambda g:g[[col_dict['expiry_col'], 
                                                                                                      col_dict['expiry_type_col']]].drop_duplicates()
    ...
    def run_backtest(self):
        ...
        for i in range(len(self.ohlcv_data) - 1):
            ...
            if self.preprocessing_required:
                ...
                previous_prices = previous_prices.rename(columns = {f'{col}_{exp}':f'{col}_FUT_{self.expiry_mapper[self.prev_timestamp][exp]}' 
                                                                       for col in rename_cols 
            ...
            proposed_position, __ ,proposed_pnl = self.update_state(finalized_actions, previous_prices=previous_prices,
                                                                    current_prices=current_prices)
...
class CopulaModel:
    def __init__(self, copula, eps = 1e-10):
        self.model = copula
        self.ecdf_x = None
        self.ecdf_y = None
        self.eps = eps
        
    def fit(self, data, bivar_cols : list):
        ...
        self.ecdf_x = ECDF(x)
        self.ecdf_y = ECDF(y)
        u = np.clip(self.ecdf_x(x), self.eps, 1 - self.eps)
        v = np.clip(self.ecdf_y(y), self.eps, 1 - self.eps)
...
class CopulaBacktester(FnOBacktester):
    ...
    def _generate_action_signal(self, historical_data_slice : pd.DataFrame, 
        ...
        if self.last_refit_date is None:
            ...
            days_since_last_refit = (current_date - self.last_refit_date).days
            need_refit = days_since_last_refit >= self.refitting_frequency
...
#     #    Make sure it's indexed by timestamp and has columns like 'open', 'high', 'low', 'close', 'volume'
#     #    for each instrument involved (e.g., 'NIFTY_FUT_open', 'NIFTY_FUT_close', ...)
#     #    Or structure it multi-indexed: (timestamp, instrument)
    
#     ohlcv_data = pd.DataFrame({
#         'NIFTY_FUT_open': [18000, 18050, 18020, 18100, 18080],
#         'NIFTY_FUT_close': [18040, 18010, 18090, 18070, 18150],
#         # ... other instruments and OHLCV ...
#     }, index=pd.to_datetime(['2023-01-01 09:15', '2023-01-01 09:16', '2023-01-01 09:17', '2023-01-01 09:18', '2023-01-01 09:19']))

#     # Add instrument columns directly for easier access in example
#     ohlcv_data['NIFTY_FUT'] = ohlcv_data.apply(lambda row: {'open': row['NIFTY_FUT_open'], 'close': row['NIFTY_FUT_close']}, axis=1)
#     # ... do this for all instruments ...


#     # 2. Define your margin rules (this is highly dependent on the broker/exchange)
#     margin_rules = {'NIFTY_FUT': {'initial': 0.12, 'maintenance': 0.06},
#                     'NIFTY_EQUITY': {'initial': 0.2, 'maintenance': 0.15}} # Very simplified example

#     # 3. Set initial capital
#     initial_capital = 1000000

#     # 4. Instantiate the backtester
#     backtester = FnOBacktester(initial_capital, ohlcv_data, margin_rules)

#     # 5. (Optional) Load or define your trained trading model/logic
#     # backtester.trained_model = load_my_model()
...
#     # 7. Analyze results
#     print("\n--- Results ---")
#     print(f"Final Available Capital: {backtester.available_capital}")
#     print(f"Final Margin Maintained: {backtester.margin_maintained}")
#     print(f"Final Position: {backtester.current_position}")
#     # print("PnL History:", backtester.pnl_history)
#     # print("Action History:", backtester.action_history)